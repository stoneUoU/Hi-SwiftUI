...
//            print("友盟推送授权状态: \(granted), 错误: \(String(describing: error?.localizedDescription))")
//        }
        
        UNUserNotificationCenter.current().requestAuthorization(options: [.alert, .sound, .badge]) { granted, error in
            if granted {
                DispatchQueue.main.async {
                    // 必须调用这个方法才会触发 didRegisterForRemoteNotificationsWithDeviceToken
